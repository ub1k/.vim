        ...
        for line in f:
            if current_class == '':
                if line[:10] == '@interface' or line[:9] == '@protocol':
                    current_class = re.match('@(interface|protocol)\s+(\w+)', line).group(2)
                elif line [:29] == 'UIKIT_EXTERN_CLASS @interface':
                    current_class = re.match('@(interface|protocol)\s+(\w+)', line[19:]).group(2)
            else:
                if line[:4] == '@end':
                    current_class = ''
